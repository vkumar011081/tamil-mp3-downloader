...
def download_links(links, base_url, out_dir: Path, chunk_size: int = 256 * 1024):
    ...
    chunk_size: number of bytes read per streamed chunk.  A larger default keeps
        progress-bar redraws from using excessive CPU while downloading.
    ...
    for idx, href in enumerate(audio_links, start=1):
        ...
        try:
            # Closing each response promptly prevents open connections from
            # accumulating across a long download session.
            with requests.get(file_url, stream=True, timeout=30) as req:
                ...
                with open(out_path, 'wb') as fh:
                    ...
                    # expected_size is the number of chunks. Round up so the
                    # final partial chunk is represented correctly.
                    if length:
                        expected_chunks = max(1, (length + chunk_size - 1) // chunk_size)
                    ...
                    # With 256 KiB chunks, this redraws about 1/1,000 as often
                    # as the old 256-byte default.
                    label = f"⬇️  [{idx}/{total_audio}] {filename} ({size_display})"
                    for chunk in progress.bar(
                        req.iter_content(chunk_size=chunk_size),
                        expected_size=expected_chunks,
                        label=label,
                    ):
                        if chunk:
                            fh.write(chunk)
